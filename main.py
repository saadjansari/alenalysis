...
from src.CalcNumXlinks import (
    PlotStateCounts,
    PlotXlinkPerFilament,
    PlotXlinkPerFilamentVsTime,
    PlotXlinkPerFilamentVsTimeMax,
)
...
from src.CalcMobility import (
    PlotMobilityFilamentVsTime, 
    PlotMobilityCrosslinkerHist,
    PlotMobilityCrosslinkerVsTime,
)
from src.CalcOrderParametersLocal import *
from src.unpickler import renamed_load
from src.CalcContactNumber import (
    PlotContactNumberHistogram,
    PlotContactNumberVsTime,
    WriteContactNumber2vtk,
)
...
from src.CalcCrosslinkerPositionOnFilament import (
...
def main(params):
    ...
    if cfg["PlotTrajectories"]:
        FData.plot_trajectories(
            params["plot_path"] / "trajectory_filament.pdf", alpha=0.3
        )
        XData.plot_trajectories(
            params["plot_path"] / "trajectory_xlinker.pdf", alpha=0.3
        )
    else:
        pass
    ...
    if cfg["PlotCrosslinkerStats"]:
        PlotStateCounts(
            FData, XData, params["plot_path"] / "graph_xlinker_num_per_state.pdf"
        )
        PlotXlinkPerFilamentVsTime(
            FData, XData, params["plot_path"] / "heatmap_xlinker_per_filament.pdf"
        )
        PlotXlinkPerFilament(
            FData, XData, params["plot_path"] / "hist_xlinker_per_filament.pdf"
        )
        PlotXlinkPerFilamentVsTimeMax(
            FData, XData, params["plot_path"] / "graph_xlinker_per_filament_max.pdf", 5
        )
    ...
    if cfg["PlotCrosslinkerLength"]:
        ...
        PlotCrosslinkerLength(
            XData, params, savepath / "graph_xlinker_length_vs_time.pdf"
        )
    else:
        pass
    ...
    if cfg["PlotExtensileMotors"]:
        PlotExtensileFilamentPairs(
            FData, XData, params, savepath / "graph_extensile_motors.pdf"
        )
    else:
        pass
    # }}}
    ...
    if cfg["PlotCorrelations"]:
        ...
        PlotContactNumberVsTime(
            FData, savepath / "heatmap_contact_number_per_filament.pdf"
        )
        PlotContactNumberHistogram(
            FData, savepath / "hist_contact_number_per_filament.pdf"
        )
        ...
        PlotLocalNematicOrderContactNumberHistogram(
            FData, savepath / "hist2_local_nematic_order_contact_number.pdf"
        )
        PlotLocalPolarOrderContactNumberHistogram(
            FData, savepath / "hist2_local_polar_order_contact_number.pdf"
        )
        PlotLocalNematicOrderLocalPolarOrderHistogram(
            FData, savepath / "hist2_local_nematic_order_local_polar_order.pdf"
        )
    ...
    if cfg["PlotLocalMaps"]:
        ...
        PlotNumberDensityXlink(
            FData, XData, savepath / "map_number_density_xlink.pdf", N=20
        )
    ...
    if cfg["ComputeFilamentCondensates"]:
        PlotFilamentCondensation(
            FData, XData, params, write2vtk=cfg["WriteFilamentCondensatesToVTK"]
        )
    else:
        pass
